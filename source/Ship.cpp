	...
	if(hyperspaceCount == HYPER_C)
	{
		...
		if(isUsingJumpDrive)
		{
			if(GetParent())
			{
				...
				auto angleoffset = (HYPER_D * 360.)/(2 * 3.141592653589793 * parentPosition.Length());
				position = target + Angle( (180 * atan2(parentPosition.Y(),parentPosition.X())/3.141592653589793 ) + 90 - angleoffset + Random::Real() * 2 * angleoffset).Unit() * parentPosition.Length();
			}
			else			
